...
export function LegalChat({ matterId }: { matterId?: string }) {
  ...
  const [scope, setScope] = useState<"matter" | "firm">(matterId ? "matter" : "firm");
  ...
  async function handleSend() {
    ...
    try {
      ...
      setMessages((p) => [
        ...p,
        { role: "assistant", content: data.answer, sources: data.sources, queryId: data.query_id },
      ]);
    } catch {
      setMessages((p) => [
        ...p,
  ...
  async function handleFeedback(msgIdx: number, rating: 1 | -1) {
    const msg = messages[msgIdx];
    if (!msg.queryId || msg.feedback) return;

    setMessages((p) => p.map((m, i) => i === msgIdx ? { ...m, feedback: rating } : m));

    const prevUser = messages[msgIdx - 1];
    await api.submitFeedback({
      query_id: msg.queryId,
      query_text: prevUser?.content ?? "",
      retrieved_chunk_ids: (msg.sources || []).map((_, i) => i.toString()),
      rating,
    }).catch(() => null);
  }
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex gap-4 px-4 py-2 border-b border-[var(--color-border)] text-xs font-body text-slate flex-wrap">
        <label className="flex items-center gap-2 cursor-pointer select-none">
          <input type="checkbox" checked={useHyde} onChange={(e) => setUseHyde(e.target.checked)} className="accent-gold" />
          HyDE
          <span className="text-slate-dark" title="Mejora búsquedas coloquiales">(?)</span>
        </label>
        {matterId && (
          <label className="flex items-center gap-2 cursor-pointer select-none">
            <input
              type="checkbox"
              checked={scope === "firm"}
              onChange={(e) => setScope(e.target.checked ? "firm" : "matter")}
              className="accent-gold"
            />
            Buscar en toda la firma
          </label>
        )}
        <span className={scope === "firm" ? "text-gold" : "text-slate-dark"}>
          {scope === "firm" ? "Buscando en toda la firma" : "Solo esta causa"}
        </span>
      </div>
        ...
        {messages.map((msg, idx) => (
          <div key={idx} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-2xl rounded-lg ${
              msg.role === "user" ? "bg-navy-light" : "bg-navy-mid border border-[var(--color-border)]"
            } p-4`}>
              ...
              {msg.role === "assistant" && (
                <div className="mt-3 flex items-center gap-3">
                  {msg.sources && msg.sources.length > 0 && (
                    <button onClick={() => toggleSources(idx)} className="text-xs font-body text-gold hover:underline">
                  ...
                  {msg.queryId && (
                    <div className="flex items-center gap-1 ml-auto">
                      <button
                        ...
                        className={`text-sm px-2 py-0.5 rounded transition-colors ${msg.feedback === 1 ? "text-green-400" : "text-slate hover:text-green-400"}`}
                      ...
                      <button
                        onClick={() => handleFeedback(idx, -1)}
                        disabled={!!msg.feedback}
                        className={`text-sm px-2 py-0.5 rounded transition-colors ${msg.feedback === -1 ? "text-red-400" : "text-slate hover:text-red-400"}`}
              ...
              {expandedSources.has(idx) && msg.sources && (
